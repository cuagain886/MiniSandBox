...
- 每完成一个可独立审查、可独立验证的小变更，必须立即创建一个 Git commit，再开始下一个小变更。
- 计划中的每个任务编号原则上对应一个独立 commit；不得把多个任务、顺手重构或无关格式化混入同一提交。
- 提交前必须运行与该小变更直接相关的聚焦测试，并保证被提交的代码、契约、测试和必要文档保持一致。
- 工作树存在其他未提交内容时，只能显式暂存当前小变更涉及的文件或补丁；不得使用 `git add .`、`git add -A` 等方式捎带用户改动。
- commit message 使用 `<type>(<scope>): <summary>` 格式，清楚描述唯一目标；修复提交不得用含糊的 `update`、`changes` 等摘要。
- 若一个任务过大而无法形成易审查的单一提交，应先拆分任务并更新计划，再逐个实现和提交；不得先堆积多个已完成变更后统一提交。
- 除非用户明确要求 squash、rebase 或合并提交，否则保留小提交边界和可追溯历史。
