...
func TestExecutionIdentityHasNoRootOrCapabilities(t *testing.T) {
	...
	if code := execAndWait(t, harness.client, firstID, "1000:1000", []string{"/bin/sh", "-c", "cat /proc/self/status > /tmp/execution-status"}); code != 0 {
		t.Fatalf("capture execution status: exit=%d", code)
	}
	status := copyRegularFile(t, harness.client, firstID, "/tmp/execution-status")
	assertRestrictedStatus(t, status, 1000, 1000)
	...
	if code := execAndWait(t, harness.client, firstID, "1000:1000", []string{"/bin/sh", "-c", readRunnerEnv}); code == 0 || code == 42 {
	...
	if code := execAndWait(t, harness.client, firstID, "1000:1000", []string{"/bin/sh", "-c", killRunner}); code != 0 {
