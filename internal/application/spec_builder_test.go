...
// TestSandboxSpecBuilderUsesServerDefaults 验证只有 image 来自请求，其余字段来自配置。
...
// TestCreateSandboxCommandHasOnlyImage 验证应用命令不接收宿主机或 Docker 控制字段。
func TestCreateSandboxCommandHasOnlyImage(t *testing.T) {
	commandType := reflect.TypeOf(CreateSandbox{})
	if commandType.NumField() != 1 || commandType.Field(0).Name != "Image" {
